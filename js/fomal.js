...
function showWelcome() {
  if (typeof ipLoacation === "undefined" || !ipLoacation.result || !ipLoacation.result.location || !ipLoacation.result.ad_info) {
    const welcomeEl = document.getElementById("welcome-info");
    if (welcomeEl) welcomeEl.innerHTML = "<b><center>欢迎来到 LunarSerenity</center></b>";
    return;
  }
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by LunarSerenity %c 你正在访问LunarSerenity",
...
function createtime() {
  ...
  let currentTimeHtml = `<div style="font-size:13px;font-weight:bold">本站已运行 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class="fas fa-heartbeat"></i><br>旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位</div>`;
